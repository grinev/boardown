...
// Its own object rather than a method on HttpFsAdapter: this one reads the
// project folder, and keeping the two surfaces apart is what stops a write path
// from ever holding it.
export class HttpProjectFileReader implements ProjectFileReader {
  constructor(private readonly base: string = PROJECT_FILE_ENDPOINT) {}

  async readFile(path: string): Promise<ProjectFileRead> {
    const res = await fetch(`${this.base}?path=${encodeURIComponent(path)}`);
    if (!res.ok) {
