...
async function main() {
  ...
  // First release: there is no previous tag to diff against, so the full commit
  // history would be noise. Emit a short placeholder to curate by hand instead.
  if (!lastTag) {
    const body = [
      buildTitle(version, kind),
      "",
      "First release of boardown. 🎉",
      "",
      "<!-- TODO: replace with curated release notes. -->",
    ].join("\n");
  ...
  lines.push(
    `Full changelog: https://github.com/${repository}/compare/${lastTag}...v${version}`,
  );
